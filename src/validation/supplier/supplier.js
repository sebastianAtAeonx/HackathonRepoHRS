...
const registerSupplier = (data, fieldsConfig) => {
  const schema = Joi.object({
    typeOfSupplier: Joi.string().valid("domestic", "international").required(),
...
const updateSupplier = (data, fieldsConfig) => {
  const schema = Joi.object({
    ...
    additionalDetails: Joi.optional().allow("", {}),
    approverComment: Joi.string().trim().allow(""),
    supplierComment: Joi.string().trim().allow(""),
  });
  return schema.validate(data);
};

const updateSupplierByAdmin = (data) => {
  const additionalDetailsSchema = Joi.object({
    nameOfCompany: Joi.array().items(Joi.number().required()),
    businessPartnerGroup: Joi.number().required(),
    reconciliationAccount: Joi.number().required(),
    vendorClass: Joi.number().required(),
    vendorSchema: Joi.number().required(),
    paymentTerms: Joi.number().required(),
    purchaseGroup: Joi.number().required(),
  });

  const itWitholdingSchema = Joi.array().items(
    Joi.object({
      taxType: Joi.string().allow(null).required(),
      taxCode: Joi.string().allow(null).required(),
      recipientType: Joi.string().allow(null).required(),
      wtSubjct: Joi.string().allow(null).required(),
    })
  );
  ...
  const schema = Joi.object({
    ...
    itWitholding: itWitholdingSchema.required(),
  });
  return schema.validate(data);
};
const sendOtp = (data) => {
...
const getIdFromPanNo = (data) => {
  const schema = Joi.object({
    panNo: Joi.string().trim(),
  });
  return schema.validate(data);
};
...
const paginateSupplierLogs = (data) => {
  const schema = Joi.object({
    offset: Joi.number().default(0),
    limit: Joi.number().default(50),
    sort: Joi.string().default("supplierName"),
    order: Joi.string().valid("asc", "desc").default("desc"),
...
const bulkDeleteSuppliers = (data) => {
  const schema = Joi.object({
...
const sendSlugLink = (data) => {
  const schema = Joi.object({
    email: Joi.string().email().required(),
    slugType: Joi.string().allow("domestic", "international").default("domestic").required(),
    departmentId: Joi.string().allow("", null).required(),
  });
  return schema.validate(data);
};

const slugToDepartment = (data) => {
  const schema = Joi.object({
...
const dynamicStatusChange = (data) => {
  const schema = Joi.object({
